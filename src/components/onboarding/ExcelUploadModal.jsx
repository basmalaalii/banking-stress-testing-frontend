...
export default function ExcelUploadModal({ isOpen, onClose, onUploadSuccess }) {
  ...
  const validateAndSetFile = (file) => {
    ...
    if (['xlsx', 'xls', 'csv'].includes(extension)) {
      ...
      setUploadError("Invalid file type. Please upload a Microsoft Excel spreadsheet (.xlsx, .xls, or .csv).");
      setFile(null);
    }
  };
  ...
  const handleDone = () => {
    if (!file) {
      setUploadError("Please select or drop a valid Excel file first.");
      return;
    }
    ...
    // Simulate premium upload & ML pipeline processing delay
    setTimeout(() => {
      setIsUploading(false);
      onUploadSuccess({
        fileName: file.name,
        fileSize: (file.size / 1024).toFixed(2) + " KB",
        bankName: file.name.toUpperCase().includes('ADIB') ? 'ADIB' : 'NBE',
      });
      onClose();
    }, 1800);
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg bg-white rounded-3xl shadow-2xl overflow-hidden border border-slate-100 z-10 animate-slide-up">
        ...
        <div className="p-6 flex flex-col justify-center items-stretch">
          ...
          {/* Centered internal upload dashed card */}
          <div className="w-full max-w-sm mx-auto">
            <form 
              ...
              className={`relative flex flex-col items-center justify-center py-10 px-4 border-2 border-dashed rounded-2xl cursor-pointer transition-all duration-300 ${
                dragActive 
                  ? "border-indigo-600 bg-indigo-50/10 shadow-inner" 
                  : file 
            ...
            >
              ...
              {/* Excel Exclamation Tooltip Icon on top-right of the internal card - brandPurple (#6E68E7) */}
              <div className="absolute top-3.5 right-3.5 group/tooltip z-20" onClick={(e) => e.stopPropagation()}>
                <div className="w-5 h-5 rounded-full bg-indigo-50 text-indigo-600 hover:bg-indigo-100 flex items-center justify-center font-black text-xs cursor-help transition-all duration-200">
                  !
                </div>
                
                {/* Tooltip Balloon (Starts exactly at purple mark and pops up above, styled in Inter/sans) */}
              ...
              {isUploading ? (
                ...
                <div className="flex flex-col items-center text-center">
                  ...
                  <p className="text-xs text-slate-400 mt-2 px-6">
                    Supports Microsoft Excel sheets (.xlsx, .xls) and CSV datasets
                  </p>
                </div>
              )}
            </form>
          ...
          {/* Centered Guideline note under the upload card - Cozy margins, Inter/sans, text-center */}
          <div className="mt-6 flex flex-col items-center justify-center text-center font-sans px-4">
            <div className="flex items-center gap-2 justify-center mb-1">
              <span className="font-extrabold text-indigo-600 bg-indigo-50 px-2.5 py-0.5 rounded text-[10px] uppercase tracking-wider">
                INFO
              </span>
            </div>
